...
CREATE DEFINER=`sangrael`@`localhost` TRIGGER crear_canasta_usuario
AFTER INSERT ON usuario
FOR EACH ROW
BEGIN
    INSERT INTO canasta (client_id, subtotal, iva, total)
...
SELECT p.id, p.idTipo, p.descripcion, p.idEquipo, p.precio, p.numeroLikes FROM producto p 
JOIN equipo e ON p.idEquipo = e.id 
JOIN tipoproducto t ON p.idTipo  = t.id
WHERE e.id in (1) AND 
t.id IN (4);

SELECT p.id, p.idTipo, p.descripcion, p.idEquipo, p.precio, p.numeroLikes FROM producto p  JOIN tipoproducto t ON p.idTipo = t.id WHERE t.id IN (3)
...
ALTER TABLE `tokens` ADD COLUMN `sesionid` bigint(20) NOT NULL;
ALTER TABLE `tokens` ADD CONSTRAINT `fk_sesion` FOREIGN KEY (sesionid) REFERENCES sesion(id);
...
INSERT INTO `usuario` (`name`, `email`, `password`, `Activo`, `Nombres`, `ApellidoPrimero`, `ApellidoSegundo`, `fechaNacimiento`) VALUES
('angel', 'angeltorralva@hotmail.com', '1234', b'1', 'Angel', 'Torres', 'Alvarez', '2024-08-29');
...
SELECT * FROM usuario WHERE email = 'angeltorralva@hotmail.com';
